...
with open("params.yaml", 'r') as fd:
    ...
    dense = int(params['keras_bert']['dense'])
...
model.compile(optimizer='rmsprop', loss='binary_crossentropy', metrics=['acc', 'AUC', tf.keras.metrics.Precision(),
...
history = model.fit(x_train, y_train,
                    ...
                    callbacks=[DvcLiveCallback(path="./bert_token_logs")])
...
model.load_weights('models/bert_token_model' + action + '.h5')
metrics = model.evaluate(x_test, y_test)
# Enregistrement des métriques plus des paramètres (mais l’enregistrement des paramètres est redondant)
df = pd.DataFrame([[x, y, z, a, b, c, d, e] for x, y, z, a, b, c, d, e in zip([metrics[1]], [metrics[2]],
                                                                              [metrics[3]],
                                                                              [metrics[4]],
                                                                              [metrics[5]],
                                                                              [metrics[6]],
...
f = open('metrics/bert/bert_metrics.json', 'w')
f.writelines(df)
f.close
db = 'bert_token_logs/'
db = 'bert_token_logs_' + action + '/'
